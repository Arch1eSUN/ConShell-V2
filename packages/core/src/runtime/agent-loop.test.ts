...
describe('AgentLoop', () => {
  ...
  // ══════════════════════════════════════════════════════════════════
  // Round 15.5: Economic Governance Runtime Integration (PolicyDecision)
  // ══════════════════════════════════════════════════════════════════

  describe('Economic Governance (Round 15.5)', () => {
    it('block decision prevents inference entirely', async () => {
      const chatFn = vi.fn();
      const router = {
        chat: chatFn,
        chatStreaming: chatFn,
        stats: () => ({ providerCount: 1, requestCount: 0, totalCost: 0, totalInputTokens: 0, totalOutputTokens: 0, survivalTier: 'thriving', primaryProvider: 'mock' }),
      } as any;
      ...
      const mockTracker = {
        assessPressure: () => ({
          ...
          reasonCodes: ['BALANCE_EXHAUSTED', 'BLOCK_POLICY_TRIGGERED'],
          explanation: 'Balance exhausted (0¢ remaining)',
          violatedScopes: ['hourly'],
          selectedActions: ['block_inference', 'log_warning'],
          maxIterationsCap: 0,
          metricsSnapshot: { balanceCents: 0, balanceRemainingPct: 0, scopeResults: [] },
          recoveryHint: 'recovery requires income',
          overrideable: false,
          overrideSource: null,
      ...
      loop.setSpendTracker(mockTracker);

      const result = await loop.processMessage('hello');
      expect(result).toContain('[Economic Governance]');
      expect(result).toContain('Execution blocked');
      // Router.chat should NEVER have been called
      expect(chatFn).not.toHaveBeenCalled();
    });

    it('caution decision injects economic guidance', async () => {
      let capturedMessages: any[] = [];
      const router = {
        chat: async function* (msgs: any[]) {
          capturedMessages = msgs;
          yield { type: 'text', text: 'response under caution' };
        },
        stats: () => ({ providerCount: 1, requestCount: 0, totalCost: 0, totalInputTokens: 0, totalOutputTokens: 0, survivalTier: 'thriving', primaryProvider: 'mock' }),
      } as any;
      ...
      const mockTracker = {
        assessPressure: () => ({
          ...
          reasonCodes: ['HOURLY_BUDGET_NEAR_LIMIT'],
          explanation: 'Economic caution — reduced iterations, cost-aware guidance injected',
          violatedScopes: ['hourly'],
          selectedActions: ['inject_guidance', 'cap_iterations', 'log_warning'],
          maxIterationsCap: 3,
          metricsSnapshot: { balanceCents: 3500, balanceRemainingPct: 70, scopeResults: [] },
          recoveryHint: null,
          overrideable: true,
          overrideSource: null,
          decisionTimestamp: new Date().toISOString(),
      ...
      expect(systemMsg?.content).toContain('[ECONOMIC CAUTION]');
    });

    it('degrade decision caps iterations to 1', async () => {
      let callCount = 0;
      const router = {
        chat: async function* () {
          callCount++;
          yield { type: 'tool_call', toolCall: { id: `tc-${callCount}`, name: 'test_tool', arguments: '{}' } };
        },
        stats: () => ({ providerCount: 1, requestCount: 0, totalCost: 0, totalInputTokens: 0, totalOutputTokens: 0, survivalTier: 'thriving', primaryProvider: 'mock' }),
      } as any;
      ...
      const loop = new AgentLoop(
        router,
        makeToolExecutor({ test_tool: async () => 'result' }),
        memory, soul, logger,
        { maxIterations: 10, defaultModel: 'test-model' },
      );

      const mockTracker = {
        assessPressure: () => ({
          level: 'degrade' as const,
          reasonCodes: ['HOURLY_BUDGET_EXCEEDED', 'DEGRADE_POLICY_TRIGGERED'],
          explanation: 'Degraded mode — single iteration, strong cost guidance',
          violatedScopes: ['hourly'],
          selectedActions: ['inject_guidance', 'cap_iterations', 'log_warning'],
          maxIterationsCap: 1,
          metricsSnapshot: { balanceCents: 1500, balanceRemainingPct: 30, scopeResults: [] },
          recoveryHint: 'Reduce spend in: hourly(85%)',
          overrideable: true,
          overrideSource: null,
          decisionTimestamp: new Date().toISOString(),
        }),
      ...
      await loop.processMessage('do something');
      // With degrade, maxIterationsCap = 1, so only 1 call to router.chat
